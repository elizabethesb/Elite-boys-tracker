...
function getNextReward(pts) {
  for (let i = REWARDS.length - 1; i >= 0; i--) if (REWARDS[i].pts > pts) return REWARDS[i];
  return null;
}
...
export default function Tracker({ user, syncEnabled }) {
  ...
  useEffect(() => {
    if (syncEnabled && user) {
      syncToSupabase(user.id, { points, logs, tasks }).catch(err => 
        console.error('Sync failed:', err)
      );
    }
  }, [points, logs, tasks, syncEnabled, user]);
    ...
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        ...
        <div style={{ fontSize: 12, color: "#444", marginTop: 4 }}>
          Points reset every 3 months · {syncEnabled && user ? "✓ Cloud synced" : "Local only"}
        </div>
        <div style={{
